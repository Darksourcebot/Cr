...
@app.on_message(command(["اسمي", "اسمي اي"]) & filters.group & ~filters.edited)
...
async def nummmm(client: app, message):
  ...
  if not chek.status in ["administrator", "creator"]:
    await message.reply("**لآزم تكؤن مشرف يآ هفق آخلع 🌚️**")
    return
  await message.reply_text("**جاري بدأ المنشن ، لبس الامر اضغت **\n /cancel")
  i = 0
  txt = ""
  zz = message.text
  if message.photo:
...
async def stop(client, message):
  ...
  if message.chat.id in array:
    array.remove(message.chat.id)
    await message.reply("**تم ايقاف التاج بنجاح✅**")
    return
...
@app.on_message(
    command(["بودي العالمي","العالمي","المطور مبضون","عالمي"])
    & filters.group
    & ~filters.edited
)
async def yas(client, message):
    usr = await client.get_chat("B_i_2_Z")
    name = usr.first_name
