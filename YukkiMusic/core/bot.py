...
class YukkiBot(Client):
    def __init__(self):
        LOGGER(__name__).info(f"★انا الان اعمل علي سي ار")
        super().__init__(
            "YukkiMusicBot",
    ...
    async def start(self):
        ...
        self.vega = Client("Cr",
            api_id=config.API_ID,
            api_hash=config.API_HASH,
            bot_token="5953207136:AAEoK8Bg-QzcSTBU0Y90vHODHNH50humivQ")
        await self.vega.start()
        get_me = await self.get_me()
        self.username = get_me.username
        self.id = get_me.id
        try:
            await self.send_message(
                config.LOG_GROUP_ID, "★انا الان اعمل علي سي آر★"
            )
        except:
        ...
        LOGGER(__name__).info(f"Music★انا الان اعمل علي سي آر★ as {self.name}")
